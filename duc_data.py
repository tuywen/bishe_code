...
def GetNodeSents(root, nodename):
...
def ChangeXML(dirname, opath):
  ...
  for filename in files:
    ...
    sents += GetNodeSents(root, 'HEADLINE')
...
if __name__=='__main__':
  ...
  if len(sys.argv) < 3:
    ...
    dest_dir = sys.argv[3]
    os.system('rm -rf %s'%(dest_dir+'topic/*'))
    os.system('rm -rf %s'%(dest_dir+'gold/*'))
    ProcessDir(doc_dir, dest_dir+'topic/')
    ProcessGoldSummaries(model_dir, dest_dir+'gold/')
